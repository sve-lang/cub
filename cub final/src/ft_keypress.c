...
/*   Updated: 2020/11/12 21:13:41 by jrosemar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/cub3d.h"

void ft_moveupdown(t_info *arg, int key) //10 lines
{
	if (key == W || key == UP)
	{
		arg->player.y -= SIZE/3 * sin(arg->player.corner);
		arg->player.x += SIZE/3 * cos(arg->player.corner);
	}
	if (key == S || key == DOWN) //движение назад
	{
		arg->player.y += SIZE/3 * sin(arg->player.corner);
		arg->player.x -= SIZE/3 * cos(arg->player.corner);
	}
...
void ft_moverightleft(t_info *arg, int key) //15 lines
{
	double corner;
 ...
 int ft_keypress(int key, t_info *arg) // 18 lines
{
